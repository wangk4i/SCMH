...
public class HttpClientUtils {
     ...
     * 判断指定URL是否能ping通
     * @param urlString url
     * @param timeOutMillSeconds 超时的毫秒数
     * @return
     */
     ...
     * http的post请求，增加异步请求头参数，传递map格式参数
     */
    private static String ajaxPost(String url, Map<String, String> dataMap, String charset) throws IOException {
        HttpPost httpPost = new HttpPost(url);
     ...
     * http的post请求，增加异步请求头参数，传递json格式参数
     */
    private static String ajaxPostJson(String url, String jsonString, String charset) throws IOException {
        HttpPost httpPost = new HttpPost(url);
        httpPost.setHeader("X-Requested-With", "XMLHttpRequest");
        //log.warn("jsonStr:{}", jsonString);
        StringEntity stringEntity = new StringEntity(jsonString, charset);// 解决中文乱码问题
        stringEntity.setContentEncoding(charset);
        stringEntity.setContentType("application/json");
        httpPost.setEntity(stringEntity);
        //log.warn("请求参数:{}", httpPost.getEntity());
        return executeRequest(httpPost, charset);
    }
    ...
    /*private static String executeRequest(HttpUriRequest httpRequest, String charset) throws IOException{
        CloseableHttpClient httpclient;
        if ("https".equals(httpRequest.getURI().getScheme())) {
            httpclient = createSSLInsecureClient();
        } else {
